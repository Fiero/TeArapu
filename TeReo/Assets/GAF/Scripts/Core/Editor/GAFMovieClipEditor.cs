...
namespace GAFEditor.Core
{
	...
	public class GAFMovieClipEditor : GAFBaseMovieClipEditor
	{
		#region Members
		...
		new public List<GAFMovieClip> targets
		{
			get
			{
				return base.targets.ToList().ConvertAll<GAFMovieClip>(target => (GAFMovieClip)target);
			}
		}

		new public GAFMovieClip target
		{
			get
			{
				return base.target as GAFMovieClip;
			}
		}
