...
public class UIDragDropItem_Custom_MG6 : MonoBehaviour
{
	public Minigame_6_MissingLetter MG6;
...
#region Common functionality
	...
	protected virtual void Awake(){
		MG6 = GameObject.Find("Minigame_6").GetComponent<Minigame_6_MissingLetter>();
	...
	void OnDragStart ()
	{
		...
		if (cloneOnDrag)
		{
			...
			UIDragDropItem_Custom_MG6 item = clone.GetComponent<UIDragDropItem_Custom_MG6>();
			item.Start();
	...
	void OnDragEnd ()
	{
		...
		//print("DROPP:" + UICamera.hoveredObject + ":");
	}
	...
	public void ForceParent(GameObject dropParent){

		this.collider.enabled = true;

		//Check if this had even been moved.
		if(mParent != null){
			OnDragDropReleaseOriginal(dropParent);
		}
	}

	public void SetLetter(int letterindex, string letter){
		correctLetterIndex = letterindex;

		SetLabelLetter(letter);

	}

	void SetLabelLetter(string letter){
		label.text = letter;
	}
	...
	protected virtual void OnDragDropRelease (GameObject surface)
	{
		if (!cloneOnDrag)
		{
			...
			//	print("GRID:" + mGrid.GetChildList().Count);

				print("GRID:" + container.GetComponentInChildren<UIGrid>().GetChildList().Count);

				//Is this container empty? If so drop here.
				if(container.GetComponentInChildren<UIGrid>().GetChildList().Count == 0){
					...
					//Is this one of the drop containers
					MG_6_DropArea dropArea = container.GetComponent<MG_6_DropArea>();
					if(dropArea != null){
					

						print("ITEM LETTER:" + correctLetterIndex);
						print("DROP LETTER:" + dropArea.GetCorrectLetter());

						if(correctLetterIndex == dropArea.GetCorrectLetter()){
							print("CORRECT");
				...
				//If anything in the container replace back on parent.
				else{
	...
	protected virtual void OnDragDropReleaseOriginal (GameObject surface)
	{
		if (!cloneOnDrag)
		{
			mTouchID = int.MinValue;
